        ...
        for (var i = 0; i < game.players.length; i++) {
          if (game.players[i].status != "dead") {
            ...
            setInterval(function()
            {
              ...
              console.log(game.players[i].score);
            },1000);
        ...
        */
        ...
        Player.find(function (err, playersList) {
            ...
            playersList.forEach(player_data => {
                playerData = {"id": player_data._id, "loginID": player_data.loginID, "pseudo": player_data.name, "score": player_data.score, "moto": player_data.motoColor};
                console.log(playerData.id, playerData.loginID, playerData.pseudo, playerData.score, playerData.moto + " test test ");
                //player_data[1].name,
                //player_data[1].score
            })
            /*
...
function collide(playerID) {
    ...
    Player.findOne({loginID: playerID}, function(err, pl_data){
        if (err){
            ...
            console.log(" Joueur Mort : " + pl_data.name, pl_data.score);
        }
    });
